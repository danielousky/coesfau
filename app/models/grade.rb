class Grade < ApplicationRecord
  ...
  def valid_to_enroll_in academic_process
    ...
    if self.enabled_enroll_process.eql?(academic_process)
      ...
      if (academic_process_before and self.enroll_academic_processes.of_academic_process(academic_process_before.id).any?) and (['regular', 'reincorporado', 'articulo3', 'retiro_semestre'].include? self.current_permanence_status)
        return true
      end
    end
    return false
  ...
  def subjects_offer_by_dependent_v2
  ...
  def export_approved_subjects_excel
    ...
    @sheet.row(5).concat ['CÓDIGO', 'NOMBRE ASIGNATURA', 'CRÉDITOS', 'SECCIÓN', 'CALIFICACIÓN']
